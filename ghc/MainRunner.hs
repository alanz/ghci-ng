...
module MainRunner (runMain) where
    ...
    case mode of
        ...
        Right postStartupMode ->
            ...
            case postStartupMode of
                ...
                Right postLoadMode ->
                    main' runI postLoadMode dflags argv3 flagWarnings

main' :: (Haskeline.Settings GHCi -> InputT GHCi () -> GHCi ())
...
main' runI postLoadMode dflags0 args flagWarnings = do
  ...
  handleSourceError (\e -> do
    ...
    case postLoadMode of
       ...
       DoInteractive          -> ghciUI runI srcs Nothing
       DoEval exprs           -> ghciUI runI srcs $ Just $ reverse exprs
       DoAbiHash              -> abiHash srcs

  liftIO $ dumpFinalStats dflags6

ghciUI :: (Haskeline.Settings GHCi -> InputT GHCi () -> GHCi ()) -> [(FilePath, Maybe Phase)] -> Maybe [String] -> Ghc ()
#ifndef GHCI
ghciUI _ _ = throwGhcException (CmdLineError "not built for interactive use")
#else
ghciUI runI = interactiveUI runI defaultGhciSettings
#endif
